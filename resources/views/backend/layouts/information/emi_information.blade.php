        ...
        <div class="container-fluid">
            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12">
                    ...
                    <div class="card">
                        <div class="header">
                            <h2><strong>EMI Information</strong></h2>
                            <i class="fas fa-toggle-on"></i>
                        </div>
                        <div class="body">
                            <form name="sectionForm" id="sectionForm" @if ($count < 1) action="{{ route('emi-create') }}" @else action="{{ route('emi-about') }}" @endif method="POST">
                            @csrf
                                <div class="row clearfix ">
                                    <div class="col-md-6 m-auto">

                                        <label for="description">EMI Information</label>
                                        <div class="form-group">
                                                <textarea name="emi_information"  class="form-control summernote"  rows="9"> @if(!empty($emi->emi_information)) {{ $emi->emi_information }} @endif </textarea>
                                            @error ('emi_information')
                                                <small class="text-danger">{{ $message }}</small>
                                            @enderror
                                        </div>
                                        <label for="meta_title">Meta Title</label>
                                        <div class="form-group" style="text-align:center">
                                           <input type="text" name="meta_title"  class="form-control" id="meta_title" @if(!empty($emi->meta_title)) value="{{ $emi->meta_title }}"  @endif>
                                            @error ('meta_title')
                                                <small class="text-danger">{{ $message }}</small>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="form-group">
                                            <textarea name="meta_description" class="form-control" id="meta_description"  rows="6">@if(!empty($emi->meta_description)) {{ $emi->meta_description }} @endif</textarea>
                                        </div>
