...
active open
@endsection
@section('warranty_active')
    active
@endsection
@section('warranty_toggled')
            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12">
                    ...
                    <div class="card">
                        ...
                        <div class="body">
                            <form name="sectionForm" id="sectionForm" @if ($count < 1) action="{{ route('warranty-create') }}" @else action="{{ route('update-warranty') }}" @endif method="POST">
                            @csrf
                                <div class="row clearfix ">
                                    <div class="col-md-6 m-auto">

                                        <label for="description">Warranty Policy</label>
                                        <div class="form-group">
                                                <textarea name="warranty_description"  class="form-control summernote"  rows="9"> @if(!empty($warranty->warranty_description)) {{ $warranty->warranty_description }} @endif </textarea>
                                            @error ('warranty_description')
                                                <small class="text-danger">{{ $message }}</small>
                                            @enderror
                                        </div>
                                        <label for="meta_title">Meta Title</label>
                                        <div class="form-group" style="text-align:center">
                                           <input type="text" name="meta_title"  class="form-control" id="meta_title" @if(!empty($warranty->meta_title)) value="{{ $warranty->meta_title }}"  @endif>
                                            @error ('meta_title')
                                                <small class="text-danger">{{ $message }}</small>
                                            @enderror
                                        </div>
                                        ...
                                        <div class="form-group">
                                            <textarea name="meta_description" class="form-control" id="meta_description"  rows="6">@if(!empty($warranty->meta_description)) {{ $warranty->meta_description }} @endif</textarea>
                                        </div>
